...
const cercle__button = document.querySelectorAll(
  ".rond1, .rond2, .rond3, .rond4, .rond5"
);
const button__color = document.querySelectorAll(
...
const theme__button1 = document.querySelector(".theme1");
...
const navbar__button = document.querySelector(".new__nav__bar");
const nav__container = document.querySelector(".new__nav__bar__container");
...
cercle__button.forEach((element) => {
  element.addEventListener("click", function () {
    ...
    const mybuttonindex = Array.from(cercle__button).findIndex(
      (el) => el === element
    );
...
button__color.forEach((element) => {
  element.addEventListener("click", function () {
    ...
    const mycolorindex = Array.from(button__color).findIndex(
      (el) => el === element
    );
...
theme__button1.addEventListener("click", function () {
  custom__page.classList.add("customize__visible");
});
...
let pressed__color = gsc.getPropertyValue("--press-color");
...
button__color.forEach((element) => {
  element.addEventListener("click", function () {
    ...
    if (ind === 0) {
      ...
      root.style.setProperty("--press-color", "#7e5deb");
    }
    if (ind === 1) {
      root.style.setProperty("--color-primary", "#e4c747");
      root.style.setProperty("--press-color", "#f6d958");
    }
    if (ind === 2) {
      root.style.setProperty("--color-primary", "#dd4973");
      root.style.setProperty("--press-color", "#f4608a");
    }
    if (ind === 3) {
      root.style.setProperty("--color-primary", "#5ae142");
      root.style.setProperty("--press-color", "#adf7a0");
    }
    if (ind === 4) {
      root.style.setProperty("--color-primary", "#5ea4e5");
      root.style.setProperty("--press-color", "#79b7f0");
    }
  });
});
